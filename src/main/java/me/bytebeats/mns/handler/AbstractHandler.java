...
public abstract class AbstractHandler implements UISettingProvider {
    ...
    protected void updateView() {
        SwingUtilities.invokeLater(() -> {
            ...
            // Ensure table header uses a font that supports Chinese characters
            jTable.getTableHeader().setFont(new Font(jTable.getTableHeader().getFont().getName(), Font.BOLD, jTable.getTableHeader().getFont().getSize()));
            resetTabSize();
            updateRowTextColors();
            updateTimestamp();
        });
    ...
    @Override
    public boolean showPinyinName() {
        return AppSettingState.getInstance().showPinyinName;
    }
